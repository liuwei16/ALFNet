...
#Anchor Calibration
def bbox_transform_inv_AC(boxes,deltas):
    if boxes.shape[0] == 0:
        return np.zeros((0, deltas.shape[1]), dtype=deltas.dtype)
    ...
    inds=np.where(ratios<=0.41)
    offset=0.41*h[inds]-w[inds]
    pred_boxes[inds,0]=pred_boxes[inds,0]-0.5*offset
    pred_boxes[inds,2]=pred_boxes[inds,2]+0.5*offset
    
    inds=np.where(ratios>0.41)
    offset=w[inds]/0.41-h[inds]
    pred_boxes[inds,3]=pred_boxes[inds,3]+offset
