...
def get_target_fr(all_anchors, regr_layer, img_data, C,roi_stride=10,igthre=0.5,posthre=0.7,negthre=0.5):
	...
	for i in range(regr_layer.shape[0]):
		...
		occlusion_weights = np.zeros((valid_anchors.shape[0], 1))
		if num_bboxes > 0:
			...
			occlusion_weights[fg_inds, 0] = 1-max_overlaps[fg_inds] # compute occlusion loss weights
			...
			y_alf_overlap_reg[valid_idxs, :1] = occlusion_weights
