...
def resize_image(image, gts,igs, visboxes, scale=[0.4,1.5]):
    ...
    if len(gts)>0:
        ...
        visboxes = np.asarray(visboxes, dtype=float)
        visboxes[:, 0:4:2] *= ratio
        visboxes[:, 1:4:2] *= ratio
    ...
    return image, gts, igs, visboxes

def random_crop(image, gts, igs, visboxes, crop_size, limit=8):
    ...
    if len(gts)>0:
        ...
        visboxes[:, 0:4:2] -= crop_x1
        visboxes[:, 1:4:2] -= crop_y1
        visboxes[:, 0:4:2] = np.clip(visboxes[:, 0:4:2], 0, crop_w)
        visboxes[:, 1:4:2] = np.clip(visboxes[:, 1:4:2], 0, crop_h)
        visboxes=visboxes[keep_inds]

    return cropped_image, gts, igs ,visboxes

def random_pave(image, gts, igs,  visboxes,pave_size, limit=8):
    img_height, img_width = image.shape[0:2]
    ...
    if len(gts) > 0:
        ...
        visboxes[:, 0:4:2] += pave_x
        visboxes[:, 1:4:2] += pave_y
        visboxes = visboxes[keep_inds]

    return paved_image, gts, igs, visboxes
...
def augment(img_data, c):
    ...
    if c.use_horizontal_flips and np.random.randint(0, 2) == 0:
        ...
        if len(img_data_aug['vis_bboxes']) > 0:
    ...
    visboxes=np.copy(img_data_aug['vis_bboxes'])

    img, gts, igs, visboxes = resize_image(img, gts, igs, visboxes,scale=[0.4,1.5])
    if img.shape[0]>=c.size_train[0]:
        img, gts, igs ,visboxes = random_crop(img, gts, igs, visboxes,c.size_train,limit=16)
    else:
        img, gts, igs ,visboxes = random_pave(img, gts, igs, visboxes,c.size_train,limit=16)

    img_data_aug['bboxes'] = gts
    img_data_aug['ignoreareas'] = igs
    img_data_aug['vis_bboxes'] = visboxes
