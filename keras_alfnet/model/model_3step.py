...
class Model_3step(Base_model):
	...
	def creat_model(self,opt,train_data,phase='train'):
		...
		if phase=='train':
			...
			self.model_3rd.compile(optimizer=self.optimizer, loss=[losses.cls_loss, losses.regr_loss],sample_weight_mode=None)
			...
			while True:
				try:
					...
					Y2 = bbox_process.get_target_1st(self.anchors, pred1[1], img_data, opt,
													 igthre=opt.ig_overlap, posthre=opt.pos_overlap_step2,negthre=opt.neg_overlap_step2)
					...
					Y3 = bbox_process.get_target_2nd(anchors_2nd, pred2[1], img_data, opt,
													 igthre=opt.ig_overlap, posthre=opt.pos_overlap_step3, negthre=opt.neg_overlap_step3)
